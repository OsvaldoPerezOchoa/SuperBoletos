...
$sql = $conn->prepare("SELECT id, nombre_evento, dia_evento, mes_evento, lugar_evento, boletovip_evento, 
						boletopref_evento, boletogen_evento FROM evetos WHERE activo=1");
$sql->execute();
...
/*if (!empty($_POST)) {
	...
	if (esnullusu([$usuario, $password])) {
		$error[] = "Debe llenar los campos";
	}
	if(count($errores) == 0)
	{
		$errores[] = login($usuario, $password, $conn);
	}
}
*/
if (!empty($_POST)) {
	...
	if (esnull([$nombre, $apellidos, $correo, $usuario, $password, $repassword])) {
		$error[] = "Debe llenar los campos";
	}
	...
	if (emailexiste($correo, $conn)) {
		$error[] = "EL nombre de correo $correo ya existe";
	}

	if (count($error) == 0) {
		$idclient = registrarcliente([$nombre, $apellidos, $correo], $conn);

		if ($idclient > 0) {

			require 'database/enviodecorreo.php';
			$enviar = new enviodecorreo();
			$token = gentoken();

			$pass_hash = password_hash($password, PASSWORD_DEFAULT);

			$idusuario = registrarusuario([$usuario, $pass_hash, $token, $idclient], $conn);

			if ($idusuario > 0) {
				$urlcorreo = "http://localhost/SuperBoletos/activa_cliente.php?idclient=" . $idusuario . '&token=' . $token;
				$asunto = "Activacion de cuenta en superboletos";
				$cuerpo = "Estimado $nombre: <br> para continuar con el proceso de registro es necesario que de clic en la siguiente liga <a href= '$urlcorreo'>Activar cuenta</a>";

				if ($enviar->enviaremail($correo, $asunto, $cuerpo)) {
					exit;
				}
			} else {
...
<head>
	...
	<script src="https://kit.fontawesome.com/127fce75b7.js" crossorigin="anonymous"></script>
	<!-- Unicons -->
...
<body>
	...
	<header>
		...
		<div class="search-box">
			<form>
				<input type="text" name="busquedaini" id="busquedaini" placeholder="Artista, Evento, Ciudad">
				<button type="submit" formaction="listaeventos.php"><i class="fa fa-search fa-beat"></i></button>
			</form>
		</div>
		<ul class="opciones">
	...
	<section class="home">
		...
		<div class="form_container">
			...
			<div class="form login_form">
				<form action="index.php" method="post" autocomplete="off">
					...
					<div class="input_box">
						...
						<input type="text" placeholder="Ingresa tu usuario" name="usuario-login" id="usuario-login" required />
						<i class="fa-solid fa-envelope email"></i>
					</div>
					<div class="input_box">
						<input type="password" placeholder="Ingresa tu contraseña" name="password-login" id="password-login" required />
			...
			<div class="form signup_form">
				<form action="index.php" method="post" autocomplete="off">
					<h2>Signup</h2>
					<div class="input_box">
						<input type="email" placeholder="Ingresa tu e-mail" name="correo" id="correo" required />
						<i class="fa-solid fa-envelope email"></i>
					</div>
					<div class="input_box">
					...
					<div class="input_box">
						<input type="password" placeholder="Crea una contraseña" name="password" id="password" required />
						<i class="fa-solid fa-lock password"></i>
						<i class="uil uil-eye-slash pw_hide"></i>
					</div>
					<div class="input_box">
						<input type="password" placeholder="Confirma tu contraseña" name="repassword" id="repassword" required />
						<i class="fa-solid fa-lock password"></i>
						<i class="uil uil-eye-slash pw_hide"></i>
					</div>
		...
		<?php mostrarmensaje($errores) ?>

		<!-- Carrusel de iconos -->
		<form action="listaeventos.php" method="post">
			<div class="carruselicono">
				<div class="contenedor__icono">
					<div class="carousel__contenedor__icono">
						<div class="carousel__lista__icono">
							<div class="carousel__elemento__icono">
								<div class="icono">
									<a href="listaeventos.php?tipoevento=Concierto"><i class="fa-solid fa-hand-back-fist fa-beat"></i></a>
									<span>Conciertos</span>
								</div>
							</div>
							<div class="carousel__elemento__icono">
								<div class="icono">
									<a href="listaeventos.php?tipoevento=Deportes"><i class="fa-solid fa-futbol fa-beat"></i></a>
									<span>Deportes</span>
								</div>
							</div>
							<div class="carousel__elemento__icono">
								<div class="icono">
									<a href="listaeventos.php?tipoevento=Comida"><i class="fa-solid fa-utensils fa-beat"></i></a>
									<span>Comida</span>
								</div>
							</div>
							<div class="carousel__elemento__icono">
								<div class="icono">
									<a href="listaeventos.php?tipoevento=Festival"><i class="fa-solid fa-wand-magic-sparkles fa-beat"></i></a>
									<span>Festivales</span>
								</div>
							</div>
							<div class="carousel__elemento__icono">
								<div class="icono">
									<a href="listaeventos.php?tipoevento=Teatro"><i class="fa-solid fa-masks-theater fa-beat"></i></a>
									<span>Teatro</span>
								</div>
							</div>
							<div class="carousel__elemento__icono">
								<div class="icono">
									<a href="listaeventos.php?tipoevento=Stan-up"><i class="fa-solid fa-microphone fa-beat"></i></a>
									<span>Stand-up</span>
								</div>
							</div>
							<div class="carousel__elemento__icono">
								<div class="icono">
									<a href="listaeventos.php?tipoevento=Conferencia"><i class="fa-solid fa-users fa-beat"></i></a>
									<span>Conferencias</span>
								</div>
							</div>
							<div class="carousel__elemento__icono">
								<div class="icono">
									<a href="listaeventos.php?tipoevento=Teatro"><i class="fa-solid fa-hand-back-fist fa-beat"></i></a>
									<span>Rock</span>
								</div>
							</div>
